              ...
              {selectedPayment?.permiteParcelamento && (
                <FormControl>
                  <RadioGroup value={String(installments)} onChange={(event) => setInstallments(Number(event.target.value))}>
                    {installmentOptions.map((option) => <FormControlLabel control={<Radio />} key={option} label={`${option}x de ${formatCurrency(subtotal / option)}`} value={String(option)} />)}
                  </RadioGroup>
                </FormControl>
              )}
            </Stack>
          ...
          <Paper sx={{ p: 3 }}>
            <Stack gap={2}>
              ...
              <Box display="flex" justifyContent="space-between">
                ...
                <Typography fontWeight={900} variant="h6">{formatCurrency(subtotal)}</Typography>
              </Box>
              {error && <Alert severity="error">{error}</Alert>}
              <Button
                disabled={!items.length || !addressComplete || !selectedPayment || (requiresSavedCard && !paymentMethodId) || isSubmitting}
                size="large"
                variant="contained"
